...
indir_index(uint32_t b)
{
	...
	uint32_t first_indirect_block = OSPFS_NDIRECT + OSPFS_NINDIRECT;
	if (b < first_indirect_block) {
		return 0;
	}

	return (b - first_indirect_block)/OSPFS_NINDIRECT;
}
...
remove_block(ospfs_inode_t *oi)
{
	...
	if (n < OSPFS_NDIRECT) {
		...
		//free block and set pointer to 0
	...
	else // ( n >= (OSPFS_NDIRECT + OSPFS_NINDIRECT) ) 
	{
		...
		if (indir_index(n) == 0) {
			...
			indirect_block = ospfs_block(doubly_indirect_block[0]);

			//deallocate direct block, indirect block, and double indirect block
			if (direct_index(n) == 0) {
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	//*f_pos - our position in the file - the offset from the beginning of the file where we are currently looking
	//count - the amount of data requested - we will read from *f_pos to *f_pos + count
	size_t filesize = oi->oi_size;
	if (count + *f_pos > filesize)
		count = filesize - *f_pos;
